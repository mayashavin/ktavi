...
const MODE_INSTRUCTIONS: Record<WritingMode, string> = {
  light:
    'Focus only on grammar, spelling, and punctuation errors. Do not suggest stylistic changes.',
  medium:
...
export function buildWritingReviewPrompt(mode: WritingMode): string {
  return `You are an expert writing reviewer for technical blog posts. Your response should be in JSON format.
...
- Category should be one of: grammar, clarity, tone, structure, diction
